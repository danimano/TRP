...
class MainPage(tk.Frame):
    ...
    def __init__(self, parent, controller, *args, **kwargs):
        ...
        layer_lists.grid(row = 2, column = 0, sticky = "n", pady = 45)

        # Displaying the "Refresh the image" button
        refresh = ttk.Button(self, text = "Refresh the figure", command = controller.refresh_figure)
        refresh_icon = ImageTk.PhotoImage(file = settings.REFRESH)
        refresh.config(image = refresh_icon, compound = "right")
        refresh.icon = refresh_icon
        refresh.grid(row = 3, column = 0, sticky = "n", ipadx = 20, ipady = 10)
        ...
        self.plot_figure.grid(row = 2, column = 1, sticky = "nsew", padx = 10, rowspan = 3)
